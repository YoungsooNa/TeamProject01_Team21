...
vector<vector<int>> trim_list(vector<vector<int>> point_list) //겹치는 case 생각 주의
{
	...
	for (int i = 0; i < 4; i++)
	{
		if (find(final_list.begin(), final_list.end(), point_list[i]) == final_list.end())
		{
			...
			{
				if (i != j && abs(final_list[j][0] - point_list[i][0]) < 3 & abs(final_list[j][1] - point_list[i][1]) < 3)
				{
					cont = 1;
					break;
				}
...
int compare_image(Mat image, int** new_img)
{
	...
	if (accuracy > 95)
	{
		cout << " 검증 완료. 올바른 도형이 맞습니다." << endl;
	}
	else
		cout << "올바른 도형이 아닙니다" << endl;
	return 0;
...
int main()
{
	...
	for (int i = 1; i <= 1; i++) {
		...
		string path = "img/circle1.jpg";
		image = imread(path, IMREAD_COLOR); // 이미지 로딩

		vector<vector<int>> point_list = find_point(image); //최외각 포인트를 찾습니다.
